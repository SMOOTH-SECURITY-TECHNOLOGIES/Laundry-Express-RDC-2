...
import { AdminPromoCode, realApi } from '../../services/real-api';
...
const CreatorName: React.FC<{ partnerId?: string }> = ({ partnerId }) => {
  ...
  if (!partnerId) {
    return <>{t('promoManagement.platform')}</>;
  }
  ...
  return (
    <div className="p-4 bg-slate-50 dark:bg-slate-700/50 border dark:border-slate-700 rounded-lg">
      <div className="flex justify-between items-start">
        ...
        >
          {promo.is_active ? t('promoManagement.active') : t('promoManagement.inactive')}
        </span>
      </div>
      <div className="mt-2 pt-2 border-t dark:border-slate-600 text-xs text-slate-600 dark:text-slate-300 space-y-1">
        <p>
          ...
          {promo.min_order_value ? ` ${t('promoManagement.minAmount', { amount: promo.min_order_value })}` : ''}
          {promo.min_order_value && promo.is_for_new_users_only ? ' &' : ''}
          {promo.is_for_new_users_only ? ` ${t('promoManagement.newUsersOnly')}` : ''}
          {!promo.min_order_value && !promo.is_for_new_users_only ? ` ${t('promoManagement.noConditions')}` : ''}
        </p>
        <p>
          <strong>{t('promoManagement.createdBy')}:</strong> <CreatorName partnerId={promo.partner_id || undefined} />
        </p>
      </div>
      <div className="mt-2 pt-2 border-t dark:border-slate-600 flex justify-end space-x-2">
        <button onClick={onEdit} className="p-2 text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/40 rounded-lg">
          <Icon name="pencil" className="w-5 h-5" />
        </button>
        <button onClick={onDelete} className="p-2 text-red-600 hover:bg-red-100 dark:hover:bg-red-900/40 rounded-lg">
...
export const PromoManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState<PromoFormState>(initialFormState);
    ...
    realApi
      ...
      .catch(() => {
        if (isMounted) {
          setPromoCodes([]);
          addNotification(
            t('promoManagement.loadError', { default: 'Failed to load promo codes.' }),
            'error'
          );
        }
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        const updated = await realApi.updatePromoCode(editingId, {
          code: formData.code,
          discount_type: formData.discountType,
          discount_value: formData.discountValue,
          min_order_value: formData.minOrderValue || null,
          is_for_new_users_only: formData.isForNewUsersOnly,
          is_active: formData.isActive,
          partner_id: formData.partnerId || null,
        });
        setPromoCodes((current) => current.map((promo) => (promo.id === updated.id ? updated : promo)));
      } else {
        const created = await realApi.createPromoCode({
          code: formData.code,
          discount_type: formData.discountType,
          discount_value: formData.discountValue,
          min_order_value: formData.minOrderValue || null,
      ...
      addNotification(
        t('promoManagement.saveError', { default: 'Failed to save promo code.' }),
        'error'
      );
    }
  };
  ...
  const handleEdit = (promo: AdminPromoCode) => {
    setFormData({
      ...
      minOrderValue: promo.min_order_value || 0,
      isForNewUsersOnly: promo.is_for_new_users_only || false,
      isActive: promo.is_active,
      partnerId: promo.partner_id || undefined,
    });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await realApi.deletePromoCode(id);
      setPromoCodes((current) => current.filter((promo) => promo.id !== id));
    } catch {
      addNotification(
        t('promoManagement.deleteError', { default: 'Failed to delete promo code.' }),
        'error'
      );
    }
  };

  const toggleStatus = async (promo: AdminPromoCode) => {
    try {
      const updated = await realApi.updatePromoCode(promo.id, { is_active: !promo.is_active });
      setPromoCodes((current) => current.map((item) => (item.id === updated.id ? updated : item)));
    } catch {
      addNotification(
        t('promoManagement.statusError', { default: 'Failed to update promo status.' }),
        'error'
      );
    }
  };
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400 max-w-3xl">
            {t('promoManagement.backendDescription', {
              default:
                'Promo codes are now managed through the backend promotions API. Pricing consumption still remains a separate validation step.',
            })}
          </p>
        </div>
        {!isFormVisible && (
      ...
      <div className="bg-blue-50 border border-blue-200 text-blue-900 dark:bg-blue-900/20 dark:border-blue-700 dark:text-blue-100 p-4 rounded-2xl">
        <p className="font-semibold">
          {t('promoManagement.backendBackedTitle', {
            default: 'Backend-backed promo management',
          })}
        </p>
        <p className="text-sm mt-1">
          {t('promoManagement.backendBackedNotice', {
            default:
              'Create, edit, status toggle, and delete operations here now persist through the backend promotions contract. Automatic pricing application is still a separate step.',
          })}
        </p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-card dark:border dark:border-slate-700">
        ...
        {isLoadingCodes ? (
          <p className="text-center text-slate-500 py-8">{t('buttons.loading')}</p>
        ) : (
          <>
                ...
                <tbody>
                  {promoCodes.map((promo) => (
                    <tr key={promo.id} className="bg-white border-b dark:bg-slate-800 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                      ...
                      <td className="px-6 py-4">{promo.discount_value}{promo.discount_type === 'percentage' ? '%' : ' $'}</td>
                      <td className="px-6 py-4 text-xs">
                        {promo.min_order_value ? <span>{t('promoManagement.minAmount', { amount: promo.min_order_value })}</span> : ''}
                        {promo.min_order_value && promo.is_for_new_users_only ? <br /> : ''}
                        {promo.is_for_new_users_only ? <span>{t('promoManagement.newUsersOnly')}</span> : ''}
                        {!promo.min_order_value && !promo.is_for_new_users_only ? <span>{t('promoManagement.noConditions')}</span> : ''}
                      </td>
                      <td className="px-6 py-4 text-xs font-semibold"><CreatorName partnerId={promo.partner_id || undefined} /></td>
                      <td className="px-6 py-4">
                        <span
                          onClick={() => toggleStatus(promo)}
                          className={`px-2 py-1 rounded-full text-xs font-semibold cursor-pointer ${promo.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-200'}`}
                        >
                          {promo.is_active ? t('promoManagement.active') : t('promoManagement.inactive')}
                        </span>
                      </td>
