...
import { FeaturedReviews } from '../components/order-marketplace/FeaturedReviews';

export const HomePage: React.FC = () => {
  const { user, orderHistory, partners, reviews, formatPrice, setCurrentPage, resetOrderDraft } = useAppContext();
  ...
  const getStatusColor = (status: OrderStatus): { text: string; bg: string; dot: string } => {
    const map: Record<string, { text: string; bg: string; dot: string }> = {
      [OrderStatus.AWAITING_CONFIRMATION]: { text: 'text-[#FF7A00]', bg: 'bg-orange-50 dark:bg-orange-900/30', dot: 'bg-[#FF7A00]' },
      [OrderStatus.CONFIRMED]: { text: 'text-brand-blue', bg: 'bg-blue-50 dark:bg-blue-900/30', dot: 'bg-brand-blue' },
      [OrderStatus.READY_FOR_PICKUP]: { text: 'text-brand-blue', bg: 'bg-blue-50 dark:bg-blue-900/30', dot: 'bg-brand-blue' },
      [OrderStatus.PICKUP]: { text: 'text-brand-blue', bg: 'bg-blue-50 dark:bg-blue-900/30', dot: 'bg-brand-blue' },
      [OrderStatus.PROCESSING]: { text: 'text-brand-blue', bg: 'bg-blue-50 dark:bg-blue-900/30', dot: 'bg-brand-blue' },
      [OrderStatus.READY_FOR_DELIVERY]: { text: 'text-purple-600 dark:text-purple-300', bg: 'bg-purple-50 dark:bg-purple-900/30', dot: 'bg-purple-500' },
      [OrderStatus.DELIVERY]: { text: 'text-purple-600 dark:text-purple-300', bg: 'bg-purple-50 dark:bg-purple-900/30', dot: 'bg-purple-500' },
      [OrderStatus.COMPLETED]: { text: 'text-[#22C55E]', bg: 'bg-green-50 dark:bg-green-900/30', dot: 'bg-[#22C55E]' },
      [OrderStatus.REJECTED]: { text: 'text-red-500', bg: 'bg-red-50 dark:bg-red-900/30', dot: 'bg-red-500' },
      [OrderStatus.DELAYED]: { text: 'text-[#FF7A00]', bg: 'bg-orange-50 dark:bg-orange-900/30', dot: 'bg-[#FF7A00]' },
    };
    return map[status] || { text: 'text-content-muted', bg: 'bg-surface-muted', dot: 'bg-slate-400' };
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12 animate-fade-in">
      ...
      ═══════════════════════════════════════════ */}
      <section>
        <h2 className="text-lg font-bold text-content-primary mb-3">Actions rapides</h2>
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-1 px-1">
          {[
            ...
            <button
              key={i}
              onClick={item.action}
              className="flex-shrink-0 w-28 sm:w-32 bg-surface-card rounded-2xl border border-surface-border-subtle p-4 hover:shadow-md transition-all duration-200 text-center group hover:-translate-y-0.5"
            >
              ...
              <p className="text-xs font-bold text-content-primary">{item.label}</p>
              <p className="text-[10px] text-content-faint mt-0.5">{item.desc}</p>
            </button>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {[
            ...
            { icon: 'calendar', label: 'Client depuis', value: clientSince, color: 'text-slate-600', bg: 'bg-surface-muted' },
          ].map((kpi, i) => (
            <div key={i} className="bg-surface-card rounded-2xl border border-surface-border-subtle p-4 hover:shadow-md transition-shadow">
      ...
      <section>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-lg font-bold text-content-primary">Commandes actives</h2>
        ...
        {activeOrders.length === 0 ? (
          <div className="bg-surface-card rounded-2xl border border-surface-border-subtle p-8 text-center">
            <div className="p-3 bg-surface-muted rounded-2xl w-fit mx-auto mb-4">
              <Icon name="truck" className="w-8 h-8 text-content-faint" />
            </div>
            <p className="text-sm font-bold text-content-primary">Aucune commande en cours</p>
            <p className="text-xs text-content-faint mt-1">Passez votre premiere commande pour commencer !</p>
                    ...
                    {orderSteps.map((step, si) => (
                      <React.Fragment key={step.key}>
                        <div className="flex flex-col items-center">
                          ...
                          <span className={`text-[9px] sm:text-[10px] mt-1 font-medium ${si <= progressIdx ? 'text-content-primary' : 'text-slate-300'}`}>
                            {step.key}
                          </span>
                        </div>
                        {si < orderSteps.length - 1 && (
                          <div className={`flex-1 h-0.5 rounded-full mt-[-14px] sm:mt-[-16px] ${
                            si < progressIdx ? 'bg-[#22C55E]' : 'bg-surface-muted'
                          }`} />
                        )}
                      </React.Fragment>
                    ))}
      ...
      ═══════════════════════════════════════════ */}
      <section>
        <div className="bg-surface-card rounded-2xl border border-surface-border-subtle p-5 sm:p-6">
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2.5 bg-purple-500/10 rounded-xl">
              <Icon name="gift" className="w-6 h-6 text-purple-600" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-content-primary">Parrainez vos amis</h2>
              <p className="text-xs text-content-faint">Invitez vos proches et gagnez des points</p>
            </div>
          </div>

          {/* Referral Code */}
          <div className="bg-surface-muted rounded-xl p-4 mb-4">
            <p className="text-[10px] uppercase tracking-wider text-content-faint font-bold mb-1">Votre code parrain</p>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            {[
              ...
              <div key={i} className="text-center bg-surface-muted rounded-xl p-3">
                <Icon name={stat.icon as any} className="w-4 h-4 text-purple-500 mx-auto mb-1" />
                <p className="text-lg font-extrabold text-content-primary">{stat.value}</p>
                <p className="text-[10px] text-content-faint">{stat.label}</p>
              </div>
            ))}
          </div>
      ...
      <section>
        <div className="flex justify-between items-center mb-3">
          <h2 className="text-lg font-bold text-content-primary">Partenaires populaires</h2>
        ...
        {featuredPartners.length === 0 ? (
          <div className="bg-surface-card rounded-2xl border border-surface-border-subtle p-8 text-center">
            <div className="p-3 bg-surface-muted rounded-2xl w-fit mx-auto mb-4">
              <Icon name="heart" className="w-8 h-8 text-content-faint" />
            </div>
            <p className="text-sm font-bold text-content-primary">Aucun partenaire pour le moment</p>
            <p className="text-xs text-content-faint mt-1">Les partenaires bientot disponibles pres de chez vous.</p>
          </div>
        ) : (
            ...
            {featuredPartners.map(partner => {
              ...
              return (
                <button
                  ...
                  className="flex-shrink-0 w-44 sm:w-52 bg-surface-card rounded-2xl border border-surface-border-subtle overflow-hidden hover:shadow-md transition-all duration-300 group hover:-translate-y-0.5 text-left"
                >
                  <div className="relative h-28 overflow-hidden bg-gradient-to-br from-cyan-100 via-surface-card to-emerald-100 dark:from-slate-700 dark:via-surface-card dark:to-slate-800">
                    {partnerImageUrl ? (
                      <img
                        src={partnerImageUrl}
                        alt={partner.name}
                  ...
                  <div className="p-3">
                    ...
                    <div className="flex items-center gap-1 mt-1.5">
                      ...
                      <span className="text-xs font-bold text-content-primary">{partner.rating}</span>
                      <span className="text-[10px] text-content-faint">({partner.reviewCount})</span>
                    </div>
                    <div className="flex items-center gap-1 mt-1 text-content-faint">
      ...
      ═══════════════════════════════════════════ */}
      <section>
        <h2 className="text-lg font-bold text-content-primary mb-3">Pourquoi nous choisir</h2>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="bg-surface-card rounded-2xl border border-surface-border-subtle p-4 text-center hover:shadow-md transition-shadow group hover:-translate-y-0.5 duration-200">
              ...
              <h3 className="text-xs font-bold text-content-primary mb-1">{item.title}</h3>
              <p className="text-[10px] text-content-faint">{item.desc}</p>
            </div>
          ))}
        </div>
      </section>
