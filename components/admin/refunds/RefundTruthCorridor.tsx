...
import type { RefundTruthCorridor as CorridorType } from '../../../lib/admin/refunds-types';
...
export function RefundTruthCorridor({ corridors }: { corridors: CorridorType[] }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
      <div className="flex items-center gap-2 mb-4"><Icon name="shield" className="w-5 h-5 text-blue-600" /><h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Refund Truth Corridor</h3></div>
          ...
          return (
            <React.Fragment key={c.id}>
              ...
              <div className={`rounded-xl border ${st.border} ${st.bg} px-4 py-3 text-center min-w-[90px]`}>
                ...
                {c.issues > 0 && <p className="text-[9px] text-gray-500 mt-0.5">{c.issues} anomalie{c.issues > 1 ? 's' : ''}</p>}
              </div>
            </React.Fragment>
          );
        })}
