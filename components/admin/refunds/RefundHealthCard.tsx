...
import type { RefundHealth } from '../../../lib/admin/refunds-types';

export function RefundHealthCard({ health }: { health: RefundHealth }) {
  ...
  let offset = 0;
  const avgDays = Math.floor(health.avgProcessingHours / 24);
  const avgHours = health.avgProcessingHours % 24;

  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
      <div className="flex items-center gap-2 mb-4"><Icon name="shield-check" className="w-5 h-5 text-green-600" /><h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Refund Health</h3></div>
      <div className="flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="flex-1 space-y-3 w-full">
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t dark:border-slate-700">
            {[
              { label: 'Demandes ouvertes', value: String(health.openRequests) },
              { label: 'Temps moyen', value: `${avgDays}j ${avgHours}h` },
              { label: 'Montant total', value: `${health.totalAmount.toLocaleString('fr-FR')} $` },
              { label: 'Litiges associés', value: String(health.linkedDisputes) },
              { label: 'Fraudes détectées', value: String(health.fraudDetected) },
            ].map((i) => (
              <div key={i.label} className="text-xs"><p className="text-gray-500 dark:text-slate-400">{i.label}</p><p className="font-bold text-gray-900 dark:text-slate-100">{i.value}</p></div>
            ))}
          </div>
        </div>
      </div>
