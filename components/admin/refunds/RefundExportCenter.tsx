...
const SCOPES = [
  ...
  { id: 'reason' as const, label: 'Par motif' },
  { id: 'status' as const, label: 'Par statut' },
  { id: 'zone' as const, label: 'Par zone' },
];

export function RefundExportCenter({ open, onClose, onExport }: { open: boolean; onClose: () => void; onExport: (format: 'csv' | 'excel' | 'pdf' | 'json', scope: 'period' | 'partner' | 'reason' | 'status' | 'zone') => void }) {
  const [scope, setScope] = useState<'period' | 'partner' | 'reason' | 'status' | 'zone'>('period');
  if (!open) return null;
