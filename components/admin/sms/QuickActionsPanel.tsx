...
const actions = ['Créer campagne', 'Nouveau template', 'Recharger crédits', 'Voir logs', 'Export SMS', 'Ouvrir analytics'];
      ...
      <div className="space-y-2">
        {actions.map((a) => (
          <button key={a} type="button" onClick={() => onAction(a)} className="w-full flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-50 text-sm text-left">
            <Icon name="paper-plane" className="w-4 h-4 text-gray-500" />{a}
          </button>
        ))}
      </div>
    </div>
