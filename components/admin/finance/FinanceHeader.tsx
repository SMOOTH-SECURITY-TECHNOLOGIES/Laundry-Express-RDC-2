...
interface FinanceHeaderProps {
  ...
  onAddPartner: () => void;
  onAddRevenueRule: () => void;
  wsConnected?: boolean;
}

export function FinanceHeader(props: FinanceHeaderProps) {
  return (
    <header className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 py-4 px-6 -mx-6 -mt-6 rounded-t-2xl">
      <div className="flex flex-col gap-4">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Revenus</h1>
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <input value={props.search} onChange={(e) => props.onSearchChange(e.target.value)} placeholder="Rechercher commande, client, paiement..." className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={props.onAddRevenueRule} className="flex items-center gap-2 px-3 py-2 bg-[#9333EA] text-white rounded-lg text-sm font-semibold hover:bg-purple-700"><Icon name="plus" className="w-4 h-4" /> Ajouter règle revenue</button>
          </div>
        </div>
      </div>
    </header>
