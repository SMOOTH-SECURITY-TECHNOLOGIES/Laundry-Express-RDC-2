...
function statusClass(status: 'ready' | 'watch' | 'planned'): string {
...
type P0ModuleStatus = 'ready' | 'watch' | 'planned';
type P0Module = {
  name: string;
  signal: string;
  status: P0ModuleStatus;
  label: string;
  action: string;
  icon: 'gift' | 'heart' | 'share' | 'chatBubble' | 'sparkles' | 'badge-check' | 'chartBar' | 'shield';
};
...
export function GrowthEnginePanel({
  ...
  const p0Modules: P0Module[] = [
    {
      name: 'Promo Management',
      signal: `${growth.trendingOffers.length} offres tendance · ${money(growth.roi.promoRevenue)}`,
      status: growth.roi.promoRevenue > 0 || growth.trendingOffers.length > 0 ? 'ready' : 'planned',
      label: 'Connecté',
      action: 'Promotions',
      icon: 'gift' as const,
    },
    {
      name: 'Loyalty',
      signal: `${fmt(growth.retention)} clients retenus · ${money(growth.roi.loyaltyRevenue)}`,
      status: growth.roi.loyaltyRevenue > 0 || growth.retention > 0 ? 'ready' : 'planned',
      label: 'Connecté',
      action: 'Fidélité',
      icon: 'heart' as const,
    },
    {
      name: 'Referral',
      signal: `${fmt(growth.referral)} referrals · ${money(growth.roi.referralRevenue)}`,
      status: growth.referral > 0 || growth.roi.referralRevenue > 0 ? 'ready' : 'planned',
      label: 'Connecté',
      action: 'Parrainage',
      icon: 'share' as const,
    },
    {
      ...
      signal: 'Réponses avis via module Avis & Notes',
      status: 'watch',
      label: 'UI prête',
      action: 'Avis & Notes',
    ...
    },
    {
      ...
      signal: 'Sentiment client dans Avis & Notes',
      status: 'watch',
      label: 'UI prête',
      action: 'Avis & Notes',
    ...
    },
    {
      ...
      signal: 'Score partenaire exposé au dashboard admin',
      status: 'watch',
      label: 'À consolider',
      action: 'Dashboard',
    ...
    },
    {
      ...
      signal: `${fmt(growth.acquisition)} acquisitions · ROI ${growth.roi.estimatedRoi}x`,
      status: 'ready',
      label: growth.source,
      action: 'Growth Engine',
      icon: 'chartBar' as const,
    },
    {
      name: 'Anti-Fraud',
      signal: growth.promoFraudRisks.length ? `${growth.promoFraudRisks.length} risques promo à traiter` : 'Aucun risque promo détecté',
      status: growth.promoFraudRisks.some((risk) => risk.severity === 'high') ? 'watch' : 'ready',
      label: growth.promoFraudRisks.length ? 'Surveillance' : 'Stable',
      action: 'Growth Engine',
      icon: 'shield' as const,
    },
  ];
  const readyCount = p0Modules.filter((module) => module.status === 'ready').length;
  const navigateAdmin = (target: string) => {
  ...
  return (
    <section className="space-y-4" aria-label="Growth Engine">
      ...
      <div className="rounded-2xl border bg-white p-4 shadow-sm dark:bg-slate-900 sm:p-5">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2">
              <Icon name="shield-check" className="h-5 w-5 text-emerald-600" />
              <h3 className="text-sm font-semibold text-slate-950 dark:text-white">Marketing & AI Growth Platform V1 - P0</h3>
            </div>
            <p className="mt-1 max-w-3xl text-xs text-slate-500">
              Couverture des modules indispensables : promotions, fidélité, parrainage, avis, sentiment, health score, dashboard et anti-fraude.
            </p>
          </div>
          <span className="inline-flex w-fit items-center rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-700">
            {readyCount}/{p0Modules.length} modules backend-ready
          </span>
        </div>
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          {p0Modules.map((module) => (
            <article key={module.name} className="flex min-w-0 flex-col justify-between rounded-xl border border-slate-200 bg-slate-50/70 p-3 dark:border-slate-800 dark:bg-slate-800/60">
              <div className="flex min-w-0 items-start gap-3">
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-white text-slate-700 shadow-sm dark:bg-slate-900 dark:text-slate-200">
                  <Icon name={module.icon} className="h-4 w-4" />
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-bold leading-snug text-slate-950 dark:text-white">{module.name}</p>
                  <p className="mt-1 text-xs leading-5 text-slate-500">{module.signal}</p>
                </div>
              </div>
              <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${statusClass(module.status)}`}>
                  {module.label}
                </span>
                <button
                  type="button"
                  onClick={() => navigateAdmin(module.action)}
