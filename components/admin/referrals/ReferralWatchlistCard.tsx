...
export function ReferralWatchlistCard({ items, onOpen }: { items: ReferralWatchlistItem[]; onOpen?: () => void }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <Icon name="shield" className="w-5 h-5 text-red-600" />
          <h3 className="text-sm font-semibold">Referral Watchlist</h3>
        </div>
        {onOpen && (
          <button type="button" onClick={onOpen} className="text-xs text-purple-600 font-medium hover:underline">Voir tout</button>
        )}
      </div>
      {items.length === 0 ? (
