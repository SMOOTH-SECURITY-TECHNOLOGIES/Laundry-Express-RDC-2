...
export function SupportHeader({ search, onSearchChange, onRefresh, onExport, onNewTicket }: Props) {
  return (
    <header className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-700 py-4 px-6 -mx-6 -mt-6 rounded-t-2xl">
      <div className="flex flex-col gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-slate-100">Support Operations Center</h1>
          <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Tickets • Réclamations • SLA • Satisfaction client</p>
        </div>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1 max-w-md">
            <Icon name="search" className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
            <input value={search} onChange={(e) => onSearchChange(e.target.value)} placeholder="Rechercher ticket, client, commande..." className="w-full pl-9 pr-4 py-2.5 rounded-xl border text-sm outline-none focus:ring-2 focus:ring-purple-500" />
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={onNewTicket} className="flex items-center gap-2 px-3 py-2 bg-purple-600 text-white rounded-lg text-sm font-semibold"><Icon name="plus" className="w-4 h-4" /> Nouveau ticket</button>
          </div>
        </div>
      </div>
    </header>
...
function Btn({ icon, label, onClick }: { icon: string; label: string; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border hover:bg-gray-50">
