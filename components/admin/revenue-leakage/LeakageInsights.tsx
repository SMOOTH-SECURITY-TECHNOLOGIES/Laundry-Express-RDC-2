...
import type { LeakageInsight } from '../../../lib/admin/revenue-leakage-types';

export function LeakageInsights({ insights }: { insights: LeakageInsight[] }) {
  return (
      ...
      <ul className="space-y-2">
        {insights.map((i) => (
          <li key={i.id} className="flex items-start gap-2 text-xs text-gray-700 dark:text-slate-300">
            <Icon name={i.type === 'critical' ? 'fire' : i.type === 'warning' ? 'warning' : 'question-mark-circle'} className="w-4 h-4 flex-shrink-0 mt-0.5" />
            {i.text}
          </li>
        ))}
      </ul>
