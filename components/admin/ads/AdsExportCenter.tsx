...
interface Props {
  open: boolean;
  onClose: () => void;
  onExport: (format: 'csv' | 'excel' | 'pdf' | 'json', scope: 'campaigns' | 'performances' | 'roi' | 'segments' | 'attribution') => void;
}

export function AdsExportCenter({ open, onClose, onExport }: Props) {
  if (!open) return null;
            ...
            <div className="flex flex-wrap gap-2">
              {formats.map((f) => (
                <button
                  key={f}
                  type="button"
                  onClick={() => onExport(f.toLowerCase() as 'csv' | 'excel' | 'pdf' | 'json', s.key)}
                  className="px-3 py-1.5 rounded-lg text-xs font-medium border hover:bg-gray-50 dark:hover:bg-slate-800"
                >
                  {f}
                </button>
              ))}
