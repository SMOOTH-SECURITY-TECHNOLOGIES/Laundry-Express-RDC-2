...
import type { PaymentTruthCorridor as CorridorType } from '../../../lib/admin/payments-types';
...
const statusStyle = (s: string) =>
  ...
  s === 'broken' || s === 'critical' ? { bg: 'bg-red-50 dark:bg-red-900/20', border: 'border-red-200 dark:border-red-800', text: 'text-red-700 dark:text-red-300', label: s === 'broken' ? 'Broken' : 'Critical' } :
  { bg: 'bg-gray-50', border: 'border-gray-200', text: 'text-gray-700', label: s };

export function PaymentTruthCorridor({ corridors }: { corridors: CorridorType[] }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-6">
      <div className="flex items-center gap-2 mb-4"><Icon name="shield" className="w-5 h-5 text-blue-600" /><h3 className="text-sm font-semibold text-gray-900 dark:text-slate-100">Payment Truth Corridor</h3></div>
