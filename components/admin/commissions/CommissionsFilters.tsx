...
import type { CommissionFilters } from '../../../lib/admin/commissions-types';

const SERVICES = ['Tous les services', 'Pressing', 'Lavage', 'Livraison', 'Collecte', 'Express', 'Abonnement'];
...
export function CommissionsFiltersBar({ filters, onChange, onAdvanced }: { filters: CommissionFilters; onChange: (f: Partial<CommissionFilters>) => void; onAdvanced: () => void }) {
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm p-4 flex flex-wrap items-center gap-3">
      ...
      <select value={filters.service} onChange={(e) => onChange({ service: e.target.value })} className="px-3 py-1.5 rounded-lg border border-gray-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 text-xs">{SERVICES.map((s) => <option key={s} value={s}>{s}</option>)}</select>
