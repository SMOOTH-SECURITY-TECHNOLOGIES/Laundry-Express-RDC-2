...
const SCOPES = [
  { id: 'partner' as const, label: 'Par partenaire' },
  { id: 'period' as const, label: 'Par période' },
  { id: 'service' as const, label: 'Par service' },
  { id: 'zone' as const, label: 'Par zone' },
  { id: 'status' as const, label: 'Par statut' },
];

export function CommissionExportCenter({ open, onClose, onExport }: { open: boolean; onClose: () => void; onExport: (format: 'csv' | 'excel' | 'pdf' | 'json', scope: 'partner' | 'period' | 'service' | 'zone' | 'status') => void }) {
  const [scope, setScope] = useState<'partner' | 'period' | 'service' | 'zone' | 'status'>('partner');
  if (!open) return null;
