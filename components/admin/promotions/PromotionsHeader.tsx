...
export function PromotionsHeader(props: Props) {
  return (
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
          <div className="relative flex-1 max-w-md">
            ...
            <input value={props.search} onChange={(e) => props.onSearchChange(e.target.value)} placeholder="Rechercher code, campagne, segment..." className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100 text-sm focus:ring-2 focus:ring-blue-500 outline-none" />
          </div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button type="button" onClick={props.onCreatePromo} className="flex items-center gap-2 px-3 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700"><Icon name="plus" className="w-4 h-4" /> Créer promotion</button>
            <button type="button" onClick={props.onCreateCampaign} className="flex items-center gap-2 px-3 py-2 bg-[#9333EA] text-white rounded-lg text-sm font-semibold hover:bg-purple-700"><Icon name="paper-plane" className="w-4 h-4" /> Nouvelle campagne</button>
          </div>
        </div>
      </div>
    </header>
